...
{
    // 배경 로드
    SDL_Surface* temp_surface = IMG_Load("../../Resource/Map/Virtus.png");
    ...
    portal_rect_VtoN1.w = portal_width;
    portal_rect_VtoN1.h = portal_height;

    portal_rect_VtoB.w = portal_width;
    portal_rect_VtoB.h = portal_height;

    // 포탈 위치 조정(하)
    portal_rect_VtoN1.x = (WINDOW_WIDTH - portal_rect_VtoN1.w) / 2;
    portal_rect_VtoN1.y = 500;

    // 포탈 위치 조정(우)
    portal_rect_VtoB.x = 700;
    portal_rect_VtoB.y = (WINDOW_HEIGHT - portal_rect_VtoB.h) / 2;
...
void Virtus::Update(float deltaTime)
{
    ...
    // 포탈과 캐릭터 충돌 확인(N1)
    if (SDL_HasIntersection(&g_player_destination_rect, &portal_rect_VtoN1))
    {
        // 다음 맵의 플레이어 위치 수정
    ...
    // 포탈과 캐릭터 충돌 확인(B)
    if (SDL_HasIntersection(&g_player_destination_rect, &portal_rect_VtoB))
    {
        // 다음 맵의 플레이어 위치 수정
...
void Virtus::Render()
{
    ...
    SDL_RenderCopy(g_renderer, portal_texture, NULL, &portal_rect_VtoN1);
    SDL_RenderCopy(g_renderer, portal_texture, NULL, &portal_rect_VtoB);

    // 렌더러 실행
    SDL_RenderPresent(g_renderer);
}

void Virtus::HandleEvents()
{
    SDL_Event event;
