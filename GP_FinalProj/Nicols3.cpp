...
{
    // 배경 로드
    SDL_Surface* temp_surface = IMG_Load("../../Resource/Map/Nicols3.png");
    ...
    portal_rect_N3toN2.w = portal_width;
    portal_rect_N3toN2.h = portal_height;

    portal_rect_N3toMa.w = portal_width;
    portal_rect_N3toMa.h = portal_height;

    // 포탈 위치 조정(좌)
    portal_rect_N3toN2.x = 0;
    portal_rect_N3toN2.y = (WINDOW_HEIGHT - portal_rect_N3toN2.h) / 2;
    // 포탈 위치 조정(하)
    portal_rect_N3toMa.x = (WINDOW_WIDTH - portal_rect_N3toMa.w) / 2;
    portal_rect_N3toMa.y = 500;
...
void Nicols3::Update(float deltaTime)
{
    ...
    // 포탈과 캐릭터 충돌 확인(N2)
    if (SDL_HasIntersection(&g_player_destination_rect, &portal_rect_N3toN2))
    {
        // 다음 맵의 플레이어 위치 수정
    ...
    // 포탈과 캐릭터 충돌 확인
    if (SDL_HasIntersection(&g_player_destination_rect, &portal_rect_N3toMa))
    {
        // 다음 맵의 플레이어 위치 수정
        g_current_game_phase = PHASE_Maria;
...
void Nicols3::Render()
{
    ...
    SDL_RenderCopy(g_renderer, portal_texture, NULL, &portal_rect_N3toN2);
    SDL_RenderCopy(g_renderer, portal_texture, NULL, &portal_rect_N3toMa);

    // 렌더러 실행
    SDL_RenderPresent(g_renderer);
}

void Nicols3::HandleEvents()
{
    SDL_Event event;
